...
class TestParty extends FunSuite{
  test("party!"){
    ...
    val party2:List[Character]=List(char5,char6,char7,char8)//each char drops 2000XP each
    char3.Dead=true
    char4.Dead=true//so the remaining two chars on party1 get 4000XP each
    val WinningParty:Party=new Party(party1)
    val LosingParty:Party=new Party(party2)
    WinningParty.DefeatParty(LosingParty)
    assert(char1.Level==4)//every 1000XP=LevelUP
    assert(char2.Level==4)
    assert(char1.Attack==90)
